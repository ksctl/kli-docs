...
K3s is a lightweight, certified Kubernetes distribution designed for resource-constrained environments. In kli, K3s is used for creating self-managed Kubernetes clusters with the following components:
...
kli deploys K3s in a high-availability configuration with:
...
K3s in kli supports the following Container Network Interface (CNI) options:
...
The default CNI for K3s in kli is Flannel.
{{% /alert %}}
...
kli allows you to choose specific K3s versions for your cluster. The tool validates versions against available releases from the K3s GitHub repository.

![](/img/k3s-ver.png)

If no version is specified, kli will automatically use the latest stable K3s release.

## Control Plane Configuration

The K3s control plane in kli is deployed with the following optimizations:
...
K3s in kli includes several advanced features:
...
- K3s in kli is designed for self-managed clusters only
- Some K3s-specific features may require additional manual configuration

## Next Steps

After deploying a K3s cluster with kli, you can:
